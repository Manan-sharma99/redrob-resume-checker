...
# Paths
# ---------------------------------------------------------------------------
INPUT_PARQUET = Path("outputs/candidate_features.parquet")
OUTPUT_PARQUET = Path("outputs/ranked_candidates.parquet")
SUBMISSION_CSV = Path("outputs/final_submission.csv")
DIAGNOSTICS_MD = Path("outputs/ranking_diagnostics.md")
...
RELEVANCE_WEIGHT = 0.55      # Stage 2 — retrieval relevance dominates
CREDIBILITY_WEIGHT = 0.45    # Stage 3 — credibility modulates relevance
...
def run_pipeline(
    ...
    # Build a recruiter-style justification from raw feature scores.
    def _reasoning(row: pd.Series) -> str:
        import hashlib

        cid = str(row.get("candidate_id", ""))
        ret = float(row.get("retrieval_score", 0) or 0)
        evl = float(row.get("evaluation_score", 0) or 0)
        rec = float(row.get("recommendation_score", 0) or 0)
        prod = float(row.get("production_score", 0) or 0)
        spec = float(row.get("specificity_score", 0) or 0)
        ev_sup = float(row.get("evidence_support_score", 0) or 0)
        consulting = bool(row.get("consulting_only_flag", False))

        # Deterministic hash for template and adjective variation
        h = int(hashlib.md5(cid.encode()).hexdigest(), 16)

        # --- Retrieval adjective (primary signal) ---
        if ret >= 80:
            ret_adjs = ["exceptional", "outstanding", "excellent", "deep"]
        elif ret >= 60:
            ret_adjs = ["strong", "solid", "notable", "clear"]
        elif ret >= 40:
            ret_adjs = ["meaningful", "reasonable", "relevant", "moderate"]
        else:
            ret_adjs = ["emerging", "foundational", "developing", "early-stage"]
        ret_adj = ret_adjs[h % len(ret_adjs)]

        # --- Collect supporting strengths ranked by score ---
        scored_supports: list[tuple[float, str]] = []

        if evl >= 60:
            scored_supports.append((evl, "relevance evaluation expertise"))
        elif evl >= 40:
            scored_supports.append((evl, "evaluation methodology experience"))

        if rec >= 60:
            scored_supports.append((rec, "recommendation and personalization experience"))
        elif rec >= 40:
            scored_supports.append((rec, "recommendation system familiarity"))

        if prod >= 60:
            scored_supports.append((prod, "production engineering depth"))
        elif prod >= 40:
            scored_supports.append((prod, "practical implementation experience"))
        elif prod >= 25:
            scored_supports.append((prod, "hands-on engineering background"))

        if spec >= 70:
            scored_supports.append((spec, "specific technical achievements"))
        elif spec >= 50:
            scored_supports.append((spec, "reasonable technical specificity"))

        if ev_sup >= 70:
            scored_supports.append((ev_sup, "evidence-backed technical claims"))
        elif ev_sup >= 50:
            scored_supports.append((ev_sup, "supported technical experience"))

        # Pick top 2 supporting strengths by score
        scored_supports.sort(key=lambda x: -x[0])
        top_supports = [s[1] for s in scored_supports[:2]]

        if len(top_supports) == 0:
            support_phrase = "general technical background"
        elif len(top_supports) == 1:
            support_phrase = top_supports[0]
        else:
            support_phrase = f"{top_supports[0]} and {top_supports[1]}"

        # --- 10 sentence templates ---
        templates = [
            (
                f"Demonstrates {ret_adj} retrieval expertise complemented by"
                f" {support_phrase}. The profile aligns strongly with the"
                f" search and ranking focus of the role."
            ),
            (
                f"Shows {ret_adj} search and retrieval capabilities with"
                f" {support_phrase}. This combination makes the profile a"
                f" competitive fit for the position."
            ),
            (
                f"Combines {ret_adj} retrieval alignment with"
                f" {support_phrase}. The profile reflects consistent"
                f" readiness for search-focused engineering work."
            ),
            (
                f"The profile presents {ret_adj} search relevance expertise"
                f" alongside {support_phrase}. These strengths support a"
                f" well-matched candidacy for the role."
            ),
            (
                f"Brings {ret_adj} retrieval and search expertise paired"
                f" with {support_phrase}. The overall profile demonstrates"
                f" meaningful alignment with role requirements."
            ),
            (
                f"Exhibits {ret_adj} alignment with retrieval-focused"
                f" responsibilities supported by {support_phrase}. The"
                f" technical profile is well-suited for this position."
            ),
            (
                f"Reflects {ret_adj} search engineering proficiency together"
                f" with {support_phrase}. The candidate presents a"
                f" convincing fit for retrieval-oriented work."
            ),
            (
                f"Displays {ret_adj} retrieval expertise reinforced by"
                f" {support_phrase}. The profile offers credible evidence"
                f" of search engineering capability."
            ),
            (
                f"Strong match driven by {ret_adj} retrieval capabilities"
                f" and {support_phrase}. The profile demonstrates depth"
                f" aligned with the core demands of the role."
            ),
            (
                f"The candidate offers {ret_adj} search and retrieval"
                f" expertise with {support_phrase}. This profile stands out"
                f" through its alignment with the position requirements."
            ),
        ]

        text = templates[(h >> 8) % len(templates)]

        if consulting:
            text += (
                " Consulting-focused experience slightly reduces"
                " confidence despite the relevant skills demonstrated."
            )

        return text

    # top100 already carries the raw feature columns from kept (which came from df),
    # so no merge is needed — apply _reasoning directly.
    top100["reasoning"] = top100.apply(_reasoning, axis=1)
