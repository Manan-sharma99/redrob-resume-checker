...
OUTPUT_PARQUET = Path("ranked_candidates_v1.parquet")
...
def compute_relevance_score(df: pd.DataFrame) -> pd.Series:
    ...
    raw = RETRIEVAL_W * r + EVALUATION_W * e + RECOMMENDATION_W * rec
    ...
    eval_bonus = np.where(df["evaluation_score"] >= 18, 0.03, 0.0)   # ~1 term detected
    eval_bonus = np.where(df["evaluation_score"] >= 36, 0.06, eval_bonus)  # 2+ terms
